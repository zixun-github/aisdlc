...
- 若仓库存在 `.gitmodules`：执行前必须根据 `plan.md` 校验受影响且 `required` 的子仓是否已切到与根项目同名的 Spec 分支
...
- **不要用在**
  ...
  - `plan.md` 不存在，或缺少可执行的任务清单（此时回到 I1：`spec-plan`）。
...
- `plan.md` 标记为 `required` 的子仓未创建与根项目同名的 Spec 分支，或存在未声明的分支例外
- 子仓处于 detached HEAD，且 `plan.md` 未将其声明为已批准的例外
...
- **读取（渐进式披露，最少必要）**
  ...
  - `spec-context` 返回的 `SUBMODULE_SET_JSON`（如存在；用于校验分支一致性、detached HEAD、脏工作区）
- **写入（唯一状态来源）**
  - **只把状态/审计写回** `{FEATURE_DIR}/implementation/plan.md`：勾选任务、按 repo 补齐 `branch/commit/pr/changed_files`、记录关键验证结果摘要与阻塞取证路径
- **代码与配置变更**
...
2. 严格审查——识别任何会阻断执行的问题或疑虑，例如：
   ...
   - `plan.md` 已声明受影响子仓，但未写清 `required`、同名分支要求或 `exception_reason`
   - `.gitmodules` / `SUBMODULE_SET_JSON` 表明某个 `required` 子仓未切到与根项目同名的 Spec 分支，或处于 detached HEAD / 脏工作区
...
5. 回写 `plan.md`（唯一状态来源）：
   - `- [ ]` → `- [x]`
   - 按 repo 补齐 `branch/commit/pr/changed_files`
...
- 已完成任务列表（对应 Task ID）
- 验证结果摘要（关键命令 + 关键输出/信号）
- `plan.md` 已回写的位置与审计信息（按 repo 的 branch/commit/pr/changed_files）
- 未完成任务概览
- 阻塞项清单（如有）
...
- 涉及子仓时，进入 I2 前先校验同名分支；执行后按 repo 回写审计信息，不要把多个仓的结果压成单条 `commit/pr/changed_files`
...
- `spec-plan` - 生成 `{FEATURE_DIR}/implementation/plan.md`（SSOT）
